...
async def process_proxy(message: types.Message, state: FSMContext):
    ...
    # Замена https на http (библиотека aiohttp-socks не поддерживает https)
    if proxy_url.startswith('https://'):
        proxy_url = proxy_url.replace('https://', 'http://', 1)
        await message.answer("⚠️ Протокол https заменён на http (поддерживается только http/socks).")

    # Проверка допустимых протоколов
    allowed = ('http://', 'socks5://', 'socks4://')
